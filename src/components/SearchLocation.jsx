...
export default function SearchLocation({ onLocationSelected }) {
  ...
  const handleInputChange = (value) => {
    ...
    if (value.trim().length < 3) { setResults([]); setShowDropdown(false); return; }

    debounceRef.current = setTimeout(async () => {
      setIsSearching(true);
      setShowDropdown(true);
      try { setResults(await searchLocation(value)); }
      catch { setResults([]); }
  ...
  const handleKeyDown = (e) => {
    if (e.key === "Enter") {
      ...
      if (query.trim().length >= 2) {
        ...
        searchLocation(query).then(setResults).catch(() => setResults([])).finally(() => setIsSearching(false));
      }
    }
  };
    ...
    <div className="space-y-2" ref={containerRef}>
      <div className="relative">
        ...
        <input type="text" value={query} onChange={(e) => handleInputChange(e.target.value)}
          onKeyDown={handleKeyDown} onFocus={() => results.length > 0 && setShowDropdown(true)}
        ...
        {isSearching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-drx-500 animate-spin" />}
      </div>
      ...
      {showDropdown && results.length > 0 && (
        <div className="border border-gray-200 rounded-xl overflow-hidden max-h-52 overflow-y-auto shadow-lg bg-white z-10 relative">
          {results.map((result) => (
            <button key={result.place_id} type="button" onClick={() => handleSelect(result)}
      ...
      {showDropdown && !isSearching && query.length >= 3 && results.length === 0 && (
        <p className="text-xs text-gray-500 px-1">No results found. Try a different search term.</p>
      )}
