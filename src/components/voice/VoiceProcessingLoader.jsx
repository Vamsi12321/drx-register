...
export default function VoiceProcessingLoader({ stage }) {
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="space-y-3">
        {stages.map((s, index) => {
          ...
          const Icon = s.icon;
          return (
            <div key={s.key} className={`flex items-center gap-3 p-2 rounded-lg transition-all ${isActive ? "bg-drx-50 border border-drx-100" : ""}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isDone ? "bg-green-100" : isActive ? "bg-drx-100" : "bg-gray-100"}`}>
                {isDone ? (
              ...
              <div>
                <p className={`text-sm ${isDone ? "text-green-700" : isActive ? "text-drx-700 font-medium" : "text-gray-400"}`}>{s.label}</p>
                <p className={`text-xs ${isDone ? "text-green-500" : isActive ? "text-drx-500" : "text-gray-300"}`}>{s.sublabel}</p>
              </div>
              {isActive && <div className="ml-auto w-3 h-3 border-2 border-drx-500 border-t-transparent rounded-full animate-spin" />}
            </div>
          );
        })}
      </div>
